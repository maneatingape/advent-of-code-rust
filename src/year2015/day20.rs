...
//! What's more, an offline brute force search showed that for all
//! house numbers between `540_540` and `1_201_200`, there are only 46
//! record-holders; so similar to part 1, just use a pre-computed LUT.
...
fn lookup(data: &[Mapping], target: u32) -> u32 {
    ...
    data.get(idx).map(|entry| entry.house).unwrap()
}
