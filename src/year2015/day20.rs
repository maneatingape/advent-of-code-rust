...
//! ## Part Two
//!
//! For n>50, it is O(1) work per candidate to check whether the first
//! 50 elves had an integer divisor to visit n, which is faster than
//! the complexity of trying to build up candidates by prime factors.
//! What's more, an offline brute force search showed that for house
//! numbers between 1008 and 5 billion, all new record-holder sums are
//! created from n divisible by 60.
use crate::util::hash::*;
...
pub fn part2(input: &u32) -> u32 {
    ...
    // There is an upper bound presents(n) <= 11 * n * H_(50), where
    // H_ is the k-th harmonic number.  The 50th harmonic number is
    // approximately 4.499, which lets us set a reasonable lower bound
    // to start searching at; using integer math to approximate is
    // fine (it's okay if we check a few extra houses on the low end
    // due to rounding errors).  A brute force exploration up to a
    // target of 50 billion presents determined that we only need to
    // care about a house number that is a multiple of 60.
