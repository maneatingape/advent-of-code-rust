...
pub fn part1_testable(input: &[Reindeer], time: u32) -> u32 {
    input.iter().map(|&r| traveled(r, time)).max().unwrap()
}
...
pub fn part2_testable(input: &[Reindeer], time: u32) -> u32 {
    ...
    for second in 0..time {
        ...
        for (distance, &reindeer) in distances.iter_mut().zip(input) {
            let next = traveled(reindeer, second + 1);
            *distance = next;
...
fn traveled([speed, fly, rest]: Reindeer, time: u32) -> u32 {
