...
mod scalar {
    ...
    pub(super) fn enhance(input: &Input, steps: i32) -> u16 {
        ...
        pixels.bytes.iter().map(|&b| b as u16).sum()
    }
}
...
mod simd {
    ...
    pub(super) fn enhance(input: &Input, steps: i32) -> u16 {
        let (algorithm, grid) = input;
        let algorithm: Vec<_> = algorithm.iter().map(|&b| b as u16).collect();
        ...
        for y in 0..grid.height {
            for x in 0..grid.width {
                let point = Point::new(x, y);
                pixels[point + offset] = u16::from(grid[point] == b'#');
            }
        }
        ...
        for _ in 0..steps {
            ...
            for x in (start..end).step_by(LANE_WIDTH) {
                let edge = Simd::splat(if default == 0 { 0 } else { 0b111 });
                let mut above = edge;
                let mut row = edge;
    ...
    #[inline]
    fn from_grid(grid: &Grid<u16>, x: i32, y: i32) -> Vector {
